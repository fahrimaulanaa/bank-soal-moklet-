...
class Ujian_model extends CI_Model {
    ...
    {//echo $this->session->userdata('id_mp_sekarang');
        ...
        /// heading tabel
        $this->table->set_heading('No', 'ID Ujian','Nama Ujian', 'Token', 'Status', 'Dibuat', 'Aksi');

        $no = $start;
        foreach ($data as $row)
        {
            if($row->list_soal != ""){
                $btn_soal = anchor(base_url().'ujian/pilih_soal/'.$row->id_ujian,'<i class="glyphicon glyphicon-list"></i> Ubah Soal',array('class' => 'btn btn-primary btn-sm')).' ';
                ...
                $btn_soal = anchor(base_url().'ujian/pilih_soal/'.$row->id_ujian,'<i class="glyphicon glyphicon-list"></i> Pilih Soal &nbsp;',array('class' => 'btn btn-warning btn-sm')).' ';
                //$btn_lihat = '';
            }

            if($row->status == '1'){
            	$status = '<span class="btn btn-success btn-xs">Aktif</span>';
                $btn_aktifkan = '<a href="'.base_url().'ujian/nonaktifkan/'.$row->id_ujian.'" class="btn btn-default btn-sm"><i class="glyphicon glyphicon-check"></i> Non Aktifkan</a>';
            } else {
            	$status = '<span class="btn btn-default btn-xs">Tidak Aktif</span>';
                $btn_aktifkan = '<a href="'.base_url().'ujian/aktifkan/'.$row->id_ujian.'" class="btn btn-success btn-sm"><i class="glyphicon glyphicon-check"></i> Aktifkan</a>';
            }
            ...
            $this->table->add_row(
                ...
                $btn_aktifkan.' '.anchor(base_url().'ujian/edit/'.$row->id_ujian,'<i class="glyphicon glyphicon-pencil"></i> Edit',array('class' => 'btn btn-info btn-sm')).' '.
                $btn_soal.
                //$btn_lihat.
                anchor(base_url().'ujian/expor_tampil/'.$row->id_ujian,'<i class="glyphicon glyphicon-download"></i> Kartu Soal',array('class' => 'btn btn-default', 'target' => '_blank')).' '.
                anchor(base_url().'ujian/hapus/'.$row->id_ujian,'<i class="glyphicon glyphicon-trash"></i> Hapus',array('class'=> 'btn btn-danger btn-sm','onclick'=>"return confirm('Anda yakin akan menghapus data ini?')"))
            );
        }
    ...
    public function buat_tabel_pilih_soal($data)
    {
        ...
        /// heading tabel
        $this->table->set_heading('No','KD','Indikator','ID Soal','Soal','Jwb', 'Kesulitan','Gambar','Suara','Pilih - <a href="#" onclick="selek_semua()">Cek All</a>');
...
'.$row->soal.'</textarea>
    ...
    public function tambah()
    {
        ...
        $ujian = array(
            ...
            'alokasi_wkt' => $this->input->post('alokasi_wkt'),
    ...
    public function aktifkan($id_ujian)
    {
        $this->db->where('id_ujian', $id_ujian)->update($this->db_tabel, array('status' => 1));
    ...
    public function nonaktifkan($id_ujian)
    {
        $this->db->where('id_ujian', $id_ujian)->update($this->db_tabel, array('status' => 0));
    ...
    public function get_ujian($id_ujian)
    {
    	return $this->db->where('id_ujian', $id_ujian)
    					->get('ujian')
    					->row();
    ...
    public function get_kartu_soal($id_ujian)
    {
    	$list_soal = $this->db->where('id_ujian', $id_ujian)
  							  ->get('ujian')
  							  ->row()->list_soal;
        ...
        sort($soal);
  		...
  		for ($i=0; $i < count($soal) ; $i++) { 
  			$id_soal = explode(':', $soal[$i]);

  			//get soal
            $kartu_soal[$i]['id_soal'] = $this->get_soal_by_id($id_soal[1])->id_soal;
  			$kartu_soal[$i]['kd'] = $this->get_soal_by_id($id_soal[1])->kd;
  			$kartu_soal[$i]['indikator'] = $this->get_soal_by_id($id_soal[1])->indikator;
            $kartu_soal[$i]['soal'] = $this->get_soal_by_id($id_soal[1])->soal;
            $kartu_soal[$i]['gambar'] = $this->get_soal_by_id($id_soal[1])->gambar;
  			$kartu_soal[$i]['jawaban'] = $this->get_soal_by_id($id_soal[1])->jawaban;
  		}
